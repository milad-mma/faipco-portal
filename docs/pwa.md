...
## به‌روزرسانی خودکار (بدون نیاز به Uninstall/Reinstall)
...
با پیداشدن نسخه جدید، بدون نیاز به هیچ اقدام کاربر (Uninstall، پاک‌کردن
Cache، یا حتی Refresh دستی)، صفحه خودش یک‌بار Reload می‌شود — `localStorage`
(و در نتیجه ورود کاربر) دست‌نخورده می‌ماند، فقط کدهای فرانت‌اند تازه می‌شوند.
