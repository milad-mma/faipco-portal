...
متن اعلان همچنان متن ساده است (HTML رندر نمی‌شود)، ولی `components/LinkifiedText.jsx` لینک‌ها را به المان React تبدیل می‌کند:
- `[متن لینک](https://example.com)` ← لینک خارجی، در تب جدید (`rel="noopener noreferrer"`).
- `[متن لینک](/leave-requests)` ← مسیر داخلی پرتال با React Router؛ کلیک روی آن دیالوگ اعلان را می‌بندد.
- آدرس خام `https://...` ← خودکار لینک می‌شود (نقطه/ویرگول انتهای جمله جزو آدرس حساب نمی‌شود).

فقط `http`/`https` و مسیرهای شروع‌شده با `/` مجازند؛ هر شِمای دیگر (مثل `javascript:`) به‌صورت متن عادی نمایش داده می‌شود. در صفحه «بررسی و اعمال آپدیت»، وقتی متن لینک دارد، پیش‌نمایش زیر فیلد نمایش داده می‌شود.
