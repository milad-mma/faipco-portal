...
## کانال آپدیت (تگ یا شاخه)

`install.sh` (چه از پنل، چه دستی) طبق `UPDATE_CHANNEL` در `backend/.env` تصمیم می‌گیرد چه چیزی نصب شود:

| مقدار | چه چیزی نصب می‌شود | مناسب |
|---|---|---|
| `tag` (پیش‌فرض) | آخرین تگ Git (بالاترین نسخه با ترتیب نسخه‌ای، مثل `v1.10.0` > `v1.9.0`). commitهای بعد از تگ تا تگ بعدی نصب **نمی‌شوند**. اگر هیچ تگی نباشد، به شاخه برمی‌گردد. | سرور اصلی |
| `branch` | آخرین commit شاخه (`FAIPCO_BRANCH`، پیش‌فرض `main`) - هر push. | سرور تست |

- اولویت: متغیر محیطی `FAIPCO_UPDATE_CHANNEL` هنگام اجرای `install.sh` > مقدار موجود در `backend/.env` > `tag`. مقدار مؤثر هر اجرا در `.env` ذخیره می‌شود (چسبنده).
- تغییر کانال: خط `UPDATE_CHANNEL=` در `backend/.env` را ویرایش کنید؛ آپدیت بعدی با همان کانال انجام می‌شود. (نیازی به restart نیست - install.sh خودش می‌خواند.)
- تگ باید روی commitی باشد که به GitHub push شده. برای انتشار: commit → push → tag → push تگ (`git push origin v1.3.0`).
- **محافظ برگشت به عقب:** اگر تگ هدف از کد نصب‌شده‌ی فعلی قدیمی‌تر باشد (مثلاً سرور قبلاً با کانال `branch` جلوتر رفته)، install.sh کد را عوض نمی‌کند و هشدار می‌دهد - چون کد قدیمی‌تر روی دیتابیس جدیدتر (Migration های ناشناخته) خراب می‌شد. راه‌حل: تگ جدیدتر منتشر کنید و دوباره آپدیت بزنید.
- صفحه «بررسی و اعمال آپدیت» کانال فعلی را بالای صفحه نشان می‌دهد؛ `GET /system/version` هم `update_channel` را برمی‌گرداند.
