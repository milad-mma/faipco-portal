      ...
      */}
      ...
      <Box
        ...
        sx={{
          ...
          // ⚠️ رفع دوباره و ساده‌تر: قبلی از calc()+env() استفاده می‌کرد که
          // نظراً باید کار می‌کرد، ولی طبق بازخورد مستقیم (دوباره) هنوز کافی
          // نبود. برای رفع هرگونه شکِ احتمالی درباره پشتیبانی calc()/env()
          // در ترکیب با سیستم Spacing خودِ MUI، این‌بار یک عدد ساده و
          // به‌مراتب بزرگ‌تر (بدون calc) استفاده شد — با فاصله اطمینان
          // خیلی بیشتر از ارتفاع واقعی نوار پایین (~۶۸px).
          pb: isPersonnelNav ? "180px" : { xs: 2, md: 4 },
          // ⚠️ باگ واقعی همین‌جا پیدا شد: overflowX:"hidden" بدون تعیین
          // صریح overflowY باعث می‌شود طبق خودِ استاندارد CSS
          // (https://www.w3.org/TR/css-overflow-3/#overflow-control)،
          // overflowY به‌جای مقدار پیش‌فرض "visible" به "auto" محاسبه شود —
          // یعنی این Box (بدون این‌که قصدش را داشته باشیم) به یک Scroll
          // Container کاملاً مستقل و جداگانه تبدیل می‌شد، نه این‌که کل صفحه
          // (body) اسکرول شود. چون این Box هم‌زمان داخل یک Flex Container با
          // align-items پیش‌فرض (stretch) قرار دارد، ارتفاعش عملاً به‌اندازه
          // Viewport محدود می‌ماند و محتوای اضافه (از جمله pb بالا) داخل
          // همین Box جداگانه اسکرول می‌شد — نه صفحه اصلی؛ که رفتار
          // اسکرول لمسی مورد‌انتظار در موبایل را نمی‌داد و باعث می‌شد
          // کارت‌های پایین صفحه همیشه زیر نوار پایین (که به Viewport وصل
          // است) دیده شوند. با overflowY صریح "visible"، این Box دیگر
          // Scroll Container جداگانه نیست و کل صفحه به‌طور طبیعی اسکرول
          // می‌شود، دقیقاً مثل بدون overflowX اصلاً.
          overflowY: "visible",
          overflowX: "hidden",
        }}
      >
