...
/notices        (POST)  ایجاد اطلاعیه جدید (Draft) — نیازمند notices.create
/notices/{id}/publish (POST) انتشار اطلاعیه — نیازمند notices.create
/notices        (GET)   لیست کامل همه اطلاعیه‌ها — نیازمند notices.view (پنل Admin)
/notices/me     (GET)   اطلاعیه‌های قابل‌مشاهده برای کاربر جاری — هر کاربر لاگین‌شده
"""
...
async def create_notice(
    ...
    return await NoticeService(db).create_notice(current_user.id, payload)
...
async def publish_notice(
    notice_id: int,
    db: AsyncSession = Depends(get_db),
    _user=Depends(require_permission("notices.create")),
):
...
async def list_notices(
    db: AsyncSession = Depends(get_db),
    _user=Depends(require_permission("notices.view")),
):
